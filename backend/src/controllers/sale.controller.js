                ...
                <body>
                    <div class="card">
                        ...
                        <div class="info">
                            ...
                            <p><strong>Dirección/Entrega:</strong> ${sale.userAddress || '-'}</p>
...
export const getUnreadOrdersCount = async (req, res) => {
    try {
        const count = await saleService.getUnreadCount();
        res.json({ count });
    } catch (error) {
        console.error("Error al obtener ventas no leídas:", error);
        res.status(500).json({ error: error.message });
    }
};

export const markSaleAsRead = async (req, res) => {
    try {
        const { id } = req.params;
        await saleService.updateSale(id, { isRead: true });
        res.json({ status: "success" });
    } catch (error) {
        console.error("Error al marcar venta como leída:", error);
        res.status(500).json({ error: error.message });
    }
};
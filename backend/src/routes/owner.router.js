...
import { renderSales, createSale, updateSale, deleteSale, getSaleById, acceptSale, rejectSale, exportSalesCsv } from "../controllers/sale.controller.js";
...
router.get('/products/search', verifyFirebaseToken, searchProducts);          // ← búsqueda por nombre
...
// ── Categorías ─────────────────────────────────────────────────────────────────
router.get('/categories', verifyFirebaseToken, renderCategories);
router.post('/categories', verifyFirebaseToken, createCategory);
router.put('/categories/:id', verifyFirebaseToken, updateCategory);
router.delete('/categories/:id', verifyFirebaseToken, deleteCategory);
...
// ── Ventas ─────────────────────────────────────────────────────────────────────
// IMPORTANTE: /sales/export ANTES de /sales/:id para que Express no lo confunda
router.get('/sales/export', verifyFirebaseToken, exportSalesCsv);
...
// Aceptar / Rechazar desde el email (sin auth — acceso directo desde el mail)
router.get('/sales/:id/accept', acceptSale);
router.get('/sales/:id/reject', rejectSale);

// ── Front Settings ─────────────────────────────────────────────────────────────
router.get('/front', verifyFirebaseToken, renderFrontSettings);
router.post('/front', verifyFirebaseToken, upload.single('heroBanner'), updateFrontSettings);
router.post('/front/card-presets', verifyFirebaseToken, saveCardPreset);
router.delete('/front/card-presets/:index', verifyFirebaseToken, deleteCardPreset);

// FAQs
router.post('/front/faqs', verifyFirebaseToken, saveFaq);
router.put('/front/faqs/:index', verifyFirebaseToken, updateFaq);
router.delete('/front/faqs/:index', verifyFirebaseToken, deleteFaq);
...
// import { renderDashboard, getStats, exportDashboardCsv } from "../controllers/stats.controller.js";
// import { getUsage } from "../controllers/usage.controller.js";
// import { renderFrontSettings, updateFrontSettings, saveCardPreset, deleteCardPreset } from "../controllers/settings.controller.js";
// import { upload } from "../middlewares/multer.js";
